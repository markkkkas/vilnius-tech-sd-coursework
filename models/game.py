...
class Game:
    ...
    def move_checker(self, row: int, col: int) -> None:
        ...
        if checker.is_queen:
            ...
            max_valid_attacks = [
                (cur_row + i, cur_col + j) for i in [-2, 2] for j in [-2, 2]
            ]
        else:
            if self.turn == 0:
                max_valid_moves = [
                    (cur_row + i, cur_col + j) for i in [-1] for j in [-1, 1]
                ]
                max_valid_attacks = [
                    (cur_row + i, cur_col + j) for i in [-2] for j in [-2, 2]
                ]
            else:
                max_valid_moves = [
                    (cur_row + i, cur_col + j) for i in [1] for j in [-1, 1]
        ...
        if (row, col) in max_valid_moves:
            moved_piece: Union[int, Checker] = self.board[cur_row][
                cur_col
            ].remove_checker()
            moved_piece.row = row
            moved_piece.col = col
